...
# ============================================================
# GOVERNANCE v8: Forecast Preditivo e Priorizacao
# ============================================================

def test_editorial_forecast_endpoint_returns_risk_assessment(tmp_path: Path) -> None:
    """Forecast endpoint deve retornar risk_score, trend, drivers e recommended_focus."""
    app = create_app(settings=Settings(vm_workspace_root=tmp_path / "runtime" / "vm", vm_db_path=tmp_path / "runtime" / "vm" / "workspace.sqlite3"))
    client = TestClient(app)

    brand_id = client.post("/api/v2/brands", headers={"Idempotency-Key": "fc-b"}, json={"name": "Acme"}).json()["brand_id"]
    project_id = client.post("/api/v2/projects", headers={"Idempotency-Key": "fc-p"}, json={"brand_id": brand_id, "name": "Launch"}).json()["project_id"]
    thread_id = client.post("/api/v2/threads", headers={"Idempotency-Key": "fc-t"}, json={"brand_id": brand_id, "project_id": project_id, "title": "Planning"}).json()["thread_id"]
    
    # Criar run sem marcar golden (simula baseline_none alto = risco)
    run_id = client.post(f"/api/v2/threads/{thread_id}/workflow-runs", headers={"Idempotency-Key": "fc-run"}, json={"request_text": "Campanha", "mode": "content_calendar"}).json()["run_id"]
...
def test_editorial_forecast_returns_404_for_unknown_thread(tmp_path: Path) -> None:
    """Forecast endpoint deve retornar 404 para thread inexistente."""
    app = create_app(settings=Settings(vm_workspace_root=tmp_path / "runtime" / "vm", vm_db_path=tmp_path / "runtime" / "vm" / "workspace.sqlite3"))
    client = TestClient(app)
...
def test_editorial_recommendations_include_priority_scores(tmp_path: Path) -> None:
    """Recommendations devem incluir impact_score, effort_score, priority_score e why_priority."""
    app = create_app(settings=Settings(vm_workspace_root=tmp_path / "runtime" / "vm", vm_db_path=tmp_path / "runtime" / "vm" / "workspace.sqlite3"))
    client = TestClient(app)

    brand_id = client.post("/api/v2/brands", headers={"Idempotency-Key": "pri-b"}, json={"name": "Acme"}).json()["brand_id"]
    project_id = client.post("/api/v2/projects", headers={"Idempotency-Key": "pri-p"}, json={"brand_id": brand_id, "name": "Launch"}).json()["project_id"]
    thread_id = client.post("/api/v2/threads", headers={"Idempotency-Key": "pri-t"}, json={"brand_id": brand_id, "project_id": project_id, "title": "Planning"}).json()["thread_id"]
    
    # Criar run sem marcar golden
    run_id = client.post(f"/api/v2/threads/{thread_id}/workflow-runs", headers={"Idempotency-Key": "pri-run"}, json={"request_text": "Campanha", "mode": "content_calendar"}).json()["run_id"]
    client.get(f"/api/v2/workflow-runs/{run_id}/baseline")
    ...
    # Verificar que cada recomendacao tem os novos campos
    for rec in data["recommendations"]:
        assert "impact_score" in rec, "Missing impact_score"
...
def test_editorial_recommendations_ordered_by_priority_desc(tmp_path: Path) -> None:
    """Recommendations devem estar ordenadas por priority_score descendente."""
    app = create_app(settings=Settings(vm_workspace_root=tmp_path / "runtime" / "vm", vm_db_path=tmp_path / "runtime" / "vm" / "workspace.sqlite3"))
    client = TestClient(app)

    brand_id = client.post("/api/v2/brands", headers={"Idempotency-Key": "ord-b"}, json={"name": "Acme"}).json()["brand_id"]
    project_id = client.post("/api/v2/projects", headers={"Idempotency-Key": "ord-p"}, json={"brand_id": brand_id, "name": "Launch"}).json()["project_id"]
    thread_id = client.post("/api/v2/threads", headers={"Idempotency-Key": "ord-t"}, json={"brand_id": brand_id, "project_id": project_id, "title": "Planning"}).json()["thread_id"]
    
    # Criar múltiplas runs para gerar múltiplas recomendações
    for i in range(3):
        run_id = client.post(f"/api/v2/threads/{thread_id}/workflow-runs", headers={"Idempotency-Key": f"ord-run{i}"}, json={"request_text": f"Campanha {i}", "mode": "content_calendar"}).json()["run_id"]
        client.get(f"/api/v2/workflow-runs/{run_id}/baseline")
    ...
    # Verificar ordenação por priority_score descendente
